// 模板编译， 与 xml_compile 唯一区别是添加了对不带结束符的HTML单标签检测
...
// 不带结束符的单标签检测
fn is_html_single_tag(node_name: &[char]) -> bool {
    if 
        node_name == ['i', 'm', 'g']
        || node_name == ['i', 'n', 'p', 'u', 't']
        || node_name == ['m', 'e', 't', 'a']
            ...
            Some(node) => {
                match node {
                    ...
                    Node::Tag(mut node) => {
                        ...
                        // HTML单标签检测
                        if is_html_single_tag(node.node_name) {
                            return Some(Node::Tag(node));
                        }
                        let mut children: Children = Vec::new();
...
pub fn template_compile(raw_xml: &str) -> String {
    // 转化为字符组
